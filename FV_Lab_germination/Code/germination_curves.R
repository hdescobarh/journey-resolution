...
#' Make a grid with plots of cumulative percent distribution and
#' probability density function grouped by a list of comparison to be performed
#'
#' @param cumulative_curves_df data.frame generated by generate_hill_curves
#' @param observations_df data.frame generated by extract_observations
#' @param derivative_df data.frame generated by generate_derivative_curves
#' @param half_activation_df a data.frame with treatment half activation values
#' @param comparisons a list in which each item is a character vector
#' with the treatments to plot together
#' @param treatment_colors named vector with the color by treatment group
#'
all_comparisons_plot <- function(
    cumulative_curves_df, observations_df,
    derivative_df, half_activation_df,
    comparisons, treatment_colors) {
  # Make all the length(comparisons) * 2 plots
  all_plots <- list()
  for (current_name in names(comparisons)) {
    current <- comparisons[[current_name]]

    current_curves_df <- cumulative_curves_df[
      cumulative_curves_df$Treatment %in% current,
    ]
    current_observations_df <- observations_df[
      observations_df$Treatment %in% current,
    ]
    current_derivative_df <- derivative_df[
      derivative_df$Treatment %in% current,
    ]
    current_half_activation_df <- half_activation_df[
      half_activation_df$Treatment %in% current,
    ]

    # cumulative percentage
    cumulative_plot <- plot_cumulative(
      current_curves_df,
      current_observations_df,
      treatment_colors
    ) +
      theme(
        legend.position = "none",
        plot.margin = unit(c(15, 10, 0, 5), "pt"),
      )
    all_plots[[paste(current_name, "_cumulative", sep = "")]] <- cumulative_plot


    # Density plot
    density_plot <- plot_density(
      current_derivative_df,
      current_half_activation_df,
      treatment_colors
    ) +
      theme(
        legend.position = "none",
  ...
  to_get_legend_plot <- plot_cumulative(
    cumulative_curves_df,
    observations_df,
    treatment_colors
  ) +
    labs(
      colour = expression("Tratamientos: ")
    ) +
    theme(
      legend.position = "bottom",
      legend.box.margin = margin(0, 0, 50, 0),
      legend.key.width = unit(40, "pt"),
    ) +
    guides(color = guide_legend(override.aes = list(size = 5), nrow = 1))

  # cowplot::get_legend is not working, this is a workaround
  legend <- gtable_filter(
    ggplot_gtable(
      ggplot_build(to_get_legend_plot)
    ), "guide-box"
  )

  final_plot <- plot_grid(
    without_legend, legend,
